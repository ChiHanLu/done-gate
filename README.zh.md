...
> verify-done 把收尾的決定權交還使用者——**沒有你親自勾選，這一輪就不算完成。**
...
Claude：（寫完程式、自測後，收尾前自動觸發 verify-done）
...
即可在任何地方使用 skill 與 `/verify-status`、`/verify-log` 命令。

**B — 當專案 skill：** 把 `skills/verify-done/` 複製到專案的 `.claude/skills/`，
並加上 `CLAUDE.md` 那條常駐規則，開啟該專案時就會自動載入。

```
.
├── .claude-plugin/plugin.json   # plugin 清單
├── skills/verify-done/SKILL.md  # skill 核心邏輯
├── commands/                    # /verify-status, /verify-log
├── references/                  # 深度文件，按需載入
├── examples/                    # ACCEPTANCE.md 範本
└── CLAUDE.md                    # 常駐規則：每輪收尾前先跑 verify-done
```
...
/verify-status                     # 本輪驗收進度快照（已過／待修）
/verify-log                        # 摘要 ACCEPTANCE.md 歷史紀錄
```
